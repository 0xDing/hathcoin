...
func (n Nodes) AddNode(node *Node) bool {
	...
	if key != currentPeer.Network.Address && n[key] == nil {
...
func HandleNode(node *Node) {
	...
	for {
		...
		currentPeer.Network.IncomingMessages <- *m
	}
}
...
func (n *Network) Run() {
	...
	fmt.Println("Listening in", currentPeer.Address)
	listenCb := StartListening(currentPeer.Address)

	for {
		select {
		case node := <-listenCb:
			currentPeer.Nodes.AddNode(node)
...
func CreateConnectionsQueue() (ConnectionsQueue, NodeChannel) {
	...
	go func() {
		...
		for {
			...
			if address != currentPeer.Network.Address && currentPeer.Nodes[address] == nil {
...
func networkError(err error) {
	if err != nil && err != io.EOF {
		utils.Logger.Error("Blockchain network: ", err)
	}
}
